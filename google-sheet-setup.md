...
在庫は品目の単位（g など）で数えますが、**発注は必ずロット単位**で行います。

品目マスタに持つのは次の4つです。

| 項目 | 意味 | 米の例 |
|---|---|---|
| 基準在庫 | 常に置いておきたい量（単位はg） | 1,000 g |
| 1ロットの量（入数） | 1回頼むと届く量 | 1,000 g |
| ロットの呼び名 | 袋・箱・パックなど | 袋 |
...
基準2,500g・入数1,000g・在庫0 なら、不足2,500g → 3袋 → 3,000g → ฿105（1袋35バーツの場合）です。画面には「3 ถุง/袋（3,000 g）」と、頼む個数と実際の量が並びます。

**呼び名を設定していない品目は「ล็อต / ロット」と表示されます。** 実際の売り方が分かった品目から、設定画面で袋・箱などに変えてください。

**自動提案が出すのは基準在庫だけです。** 入数は仕入先の売り方で決まるもので、使用量から計算するものではないため、提案の対象から外しました。
